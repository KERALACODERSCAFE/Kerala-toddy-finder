...
class UserDataAPITests(TestCase):
    ...
    def test_history_post_upserts_user_shop_action(self):
        self.authenticate()

        first_response = self.client.post(
            "/api/v1/history/",
            {"shop_id": self.shop.id, "action": "visited"},
            format="json",
        )
        ...
        history = UserHistory.objects.get(user=self.user, shop=self.shop, action="visited")
        self.assertEqual(history.visit_count, 2)

    def test_shop_detail_records_authenticated_view_history(self):
        self.authenticate()

        first_response = self.client.get(f"/api/v1/shops/{self.shop.id}/")
        second_response = self.client.get(f"/api/v1/shops/{self.shop.id}/")

        self.assertEqual(first_response.status_code, 200)
        self.assertEqual(second_response.status_code, 200)
        history = UserHistory.objects.get(user=self.user, shop=self.shop, action="viewed")
        self.assertEqual(history.visit_count, 2)
