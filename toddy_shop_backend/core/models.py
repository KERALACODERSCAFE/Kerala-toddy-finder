...
class UserHistory(TimeStampMixin):
    ...
    shop = models.ForeignKey("shops.ToddyShop", on_delete=models.CASCADE, related_name="history_entries")
    action = models.CharField(max_length=20, choices=HistoryAction.choices, default=HistoryAction.VIEWED)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["user", "shop", "action"], name="unique_user_shop_history_action"),
        ]
        indexes = [
            models.Index(fields=["user", "-last_seen_at"], name="user_history_seen_idx"),
        ]

    def __str__(self):
        return f"{self.user.username} {self.action} {self.shop.name}"
